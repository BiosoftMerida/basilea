...
jQuery(document).ready ($) ->
  ...
  $('#btn_procesar').click ->
    ...
    $.ajax
      type: 'POST'
      url: '/censo_forestal/mf_fisiografia'
      dataType: 'JSON'
      data:
      ...
      success: (data) ->
        ...
        for mf, index in data[0]['masa_forestal']
           $('#tbody_resultados').append("<tr><td>"+mf.gran_paisaje+"</td><td>"+mf.paisaje+"</td><td>"+mf.sub_paisaje+
            "</td><td class = 'text-right'>"+mf.num_arboles+"</td><td class = 'text-right'>"+mf.num_especies+
             "</td><td class = 'text-right'>"+mf.area_basal+"</td><td class = 'text-right'>"+mf.volumen+"</td></tr>")
  ...
  $('#printPdf').click ->
    ...
    doc.setFontSize 14
    doc.text 40, 40, "CENSO FORESTAL - REPORTE DE LA MASA FORESTAL x FISIOGRAFÍA"
    doc.setFontSize 8
    data = []
    ...
    doc.save 'reporte_mf_xfisiografia.pdf'
